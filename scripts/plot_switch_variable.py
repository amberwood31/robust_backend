...
#    options = "3d"
    ...
    print("outliers acceptted: ", sum(outlier_analysis.iloc[2]), "/", outlier_quantity)
    ...
    print("inliers acceptted: ", sum(inlier_analysis.iloc[2]), "/", lc_edge_quantity- outlier_quantity)
    ...
    if examine_edge_option == 1:
        ...
        i = 0
        while i < (lc_edge_quantity) *(iteration+1): # including initial estimate
            line = f.readline()
            line_stripped = line.strip('\n')
            line_splitted = line_stripped.split(' ')
            if len(line_splitted) > 1:
